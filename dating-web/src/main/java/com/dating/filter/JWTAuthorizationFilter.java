...
public class JWTAuthorizationFilter extends BasicAuthenticationFilter {
	...
	private static final Logger logger = Logger
			.getLogger(JWTAuthorizationFilter.class.getName());
	...
	protected void doFilterInternal(HttpServletRequest request,
		...
		SecurityContextHolder.getContext()
				.setAuthentication(getAuthentication(authorization));
	...
	private UsernamePasswordAuthenticationToken getAuthentication(String authorization) {
		...
		try {
			...
			// 通过 token 获取用户具有的角色
			List<SimpleGrantedAuthority> userRolesByToken = JwtTokenUtils
					.getUserRolesByToken(token);
			...
			logger.warning("Request to parse JWT with invalid signature . Detail : "
